...
{
	ft_printf("line %i, colum %i: error: parameter %i ", env->line_number,
			end - env->line, i);
...
{
	ft_printf("line %i error: failed to deference label name \"%s\" ",
			inst->line_number, inst->params[index].value + offset + 1);
	ft_printf("at parameter index %i for instruction %s\n",
